import { MdxCache } from "../docs-src/lib/mdx-cache.js";
import { describe, it, expect, beforeEach } from "bun:test";
import path from "node:path";

const PAGES_DIR = path.resolve(import.meta.dirname, "../docs-src/pages");
const CONTEXT_MDX = path.join(PAGES_DIR, "api/context.mdx");
const INSTALL_MDX = path.join(PAGES_DIR, "guide/installation.mdx");
...
describe("MdxCache", () => {
  ...
  it("recompiles after invalidate()", async () => {
    const cache = freshCache();
    const first = await cache.load(CONTEXT_MDX);
    const compBefore = first.Component;
