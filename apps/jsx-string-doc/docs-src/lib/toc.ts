...
function renderTocLink(entry: TocEntry): string {
  ...
  return `<a href="#${entry.id}" class="docs-toc-link block py-1 ${padding} ${size} text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors">${escapeHtml(entry.text)}</a>`;
}
