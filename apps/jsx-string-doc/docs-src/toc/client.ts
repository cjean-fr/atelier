...
export function installScrollSpy(toc: HTMLElement, opts: ScrollSpyOptions = {}): () => void {
  ...
  function getActiveId(): string {
    ...
    for (const h of headings) {
      if (h.offsetTop <= threshold) result = h.id;
    }
    return result;
  }
  ...
  function startTracking(): void {
    ...
    const loop = (): void => { placeMarker(); rafId = requestAnimationFrame(loop); };
  ...
  const onTransitionStart  = (e: TransitionEvent) => { if (isSublistTransition(e)) startTracking(); };
  const onTransitionEnd    = (e: TransitionEvent) => { if (isSublistTransition(e)) stopTracking(); };
  const onTransitionCancel = (e: TransitionEvent) => { if (isSublistTransition(e)) stopTracking(); };
