...
async function LiveComments({ signal }: { signal: AbortSignal }) {
  const res = await fetch("https://api.example.com/comments", { signal });
  ...
  return (
    <html>
      <body>
        ...
        {/*
          The factory receives an AbortSignal tied to the request lifetime.
          Pass it to fetch() so a client disconnect cancels the pending HTTP
