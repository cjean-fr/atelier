...
```sh
npm install @cjean-fr/vite-plugin-precompile -D
...
You can also set `esbuild: { jsxImportSource: "@cjean-fr/jsx-string" }` in `vite.config.ts` instead of tsconfig. Vite reads either way and exposes it via `resolvedConfig.esbuild.jsxImportSource`.
...
The plugin uses `jsxImportSource` from Vite's resolved esbuild config (which Vite reads from `tsconfig.json` or `vite.config.ts`) to determine the runtime helpers path (`{source}/jsx-runtime`). If that isn't set either, it falls back to `@cjean-fr/jsx-string/jsx-runtime`.
...
```ts
import transformer from "@cjean-fr/vite-plugin-precompile/transformer";
