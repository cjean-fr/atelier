...
describe("ASVS 1.2.1 — Context-relevant output encoding (L1)", () => {
  ...
  it("filters unsafe CSS values from style objects", async () => {
    const html = await renderToString(
  ...
  it("filters javascript: in CSS url() from style objects", async () => {
    const html = await renderToString(
...
describe("ASVS 1.2.3 — Output encoding for JavaScript/JSON contexts (L1)", () => {
  it("prevents </script> breakout in script elements", async () => {
    ...
    // < and > are entity-encoded. The raw characters `</script>` cannot
    // appear — the parser never sees a closing script tag within content.
    expect(html).toContain("&lt;/script&gt;");
    expect(html).toContain("&lt;script&gt;");
    // The content inside <script> is entity-escaped text, not executable JS
    expect(html).toMatch(
      /^<script>&lt;\/script&gt;&lt;script&gt;alert\(1\)<\/script>$/,
    );
  });
...
describe("ASVS 1.3.12 — ReDoS-free regular expressions (L3)", () => {
  ...
  it("isSafeUrl handles adversarial input without hang", () => {
