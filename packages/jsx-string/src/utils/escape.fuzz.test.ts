...
describe("URL safety invariants (property-based)", () => {
  ...
  it("blocks vbscript: under arbitrary obfuscation", () => {
    fc.assert(
      fc.property(dangerousUrl("vbscript:"), (url) => isSafeUrl(url) === false),
      { numRuns: 1000 },
    );
  });
  ...
  it("never blocks a clearly-safe URL", () => {
    ...
    fc.assert(
      fc.property(safePrefix, tail, (p, t) => isSafeUrl(p + t) === true),
      { numRuns: 1000 },
    );
  });
