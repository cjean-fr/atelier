...
export const noReactHooks = ESLintUtils.RuleCreator.withoutDocs({
  meta: {
    ...
    schema: [],
    messages: {
      noHook: '{{name}} is not compatible with @cjean-fr/jsx-string. Static rendering only.',
      useState: 'useState is not compatible with @cjean-fr/jsx-string. Extract state as props.',
      useEffect: 'useEffect is not compatible with @cjean-fr/jsx-string. Fetch data before render.',
    },
  },
  defaultOptions: [],
  create(context) {
