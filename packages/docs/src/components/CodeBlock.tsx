...
export async function CodeBlock({ code, language = "text", label }: CodeBlockProps) {
  ...
  return (
    <div class="docs-code-block relative my-4">
      {(label ?? language) !== "text" && (
        <span class="docs-code-lang absolute top-2 right-3 text-xs text-gray-500 dark:text-gray-400 font-mono select-none">
          {label ?? language}
        </span>
      )}
      {raw(body)}
